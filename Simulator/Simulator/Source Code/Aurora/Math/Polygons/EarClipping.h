...
namespace Aurora
{
	...
	namespace Math
	{
		namespace Polygons
		{
			...
			//		//
			//		const int const sizeOfPositionCollection = positions.size();
			//		DynamicArrayIndexedVector2DDouble< sizeOfPositionCollection> remainingPositions;

			//		int index = 0;
			//		for(auto &position : positions)
			//		{

			//			remainingPositions.push_back(SharedIndexedVector2DDouble(std::make_shared<IndexedVector2DDouble>(position, index++)));
			//		}
			...
			//		///////////////////////////////////////////////////////////////////
			//		DoubleLinkedListSharedIndexedVector2DDouble::iterator remainingPositionsIterator = remainingPositions.begin();
			...
			//		WeakIndexedVector2DDouble previousNode = *remainingPositionsIterator; remainingPositionsIterator++;
			//		WeakIndexedVector2DDouble node = *remainingPositionsIterator; remainingPositionsIterator++;
			//		WeakIndexedVector2DDouble nextNode = *remainingPositionsIterator;

			//		int bailCount = remainingPositions.size() * remainingPositions.size();

			//		while (remainingPositions.size() > 3)
			//		{
			//			Vector2DDouble &p0 = previousNode.lock()->Vector();
			//			Vector2DDouble &p1 = node.lock()->Vector();
			//			Vector2DDouble &p2 = nextNode.lock()->Vector();
			...
			//				for (DoubleLinkedListSharedIndexedVector2DDouble::iterator n = ((remainingPositionsIterator != remainingPositions.end()) ? remainingPositionsIterator : remainingPositions.begin());
			//					// Notice here we check that the previous node and present node by address
			//					*n != previousNode.lock();
			//				// Go the next element, check if it is not the last element(empty element or the end of the list), if not then process the current element or start from the begining
			...
			//					indices->AddTriangle(TriangleIndicesUnsignedInt(previousNode.lock()->Index(), node.lock()->Index(), nextNode.lock()->Index()));
