...
namespace Aurora {
	namespace Renderer {
		namespace Buffers {
			...
			template<typename T>
			std::shared_ptr<UniqueDynamicTypeVector<T>> Aurora::Renderer::Buffers::WritePixelBuffer<T>::CopyToSystemMemory()
			{
				return CopyToSystemMemory(0, SizeInBytes);
			}

			template<typename T>
			void Aurora::Renderer::Buffers::WritePixelBuffer<T>::CopyFromSystemMemory(const UniqueDynamicTypeVector<T> &bufferInSystemMemory, int destinationOffsetInBytes)
			{
			...
			template<typename T>
			void Aurora::Renderer::Buffers::WritePixelBuffer<T>::CopyFromSystemMemory(const UniqueDynamicTypeVector<T> &bufferInSystemMemory)
			{
