...
namespace Aurora {
	namespace Renderer {
		namespace Buffers {
			...
			template<typename T>
			std::shared_ptr<std::vector<T>> Aurora::Renderer::Buffers::UniformBuffer<T>::CopyToSystemMemory()
			{
				return CopyToSystemMemory(0, SizeInBytes);
			}

			template<typename T>
			void Aurora::Renderer::Buffers::UniformBuffer<T>::CopyFromSystemMemory(const std::vector<T> &bufferInSystemMemory, int destinationOffsetInBytes)
			{
			...
			template<typename T>
			void Aurora::Renderer::Buffers::UniformBuffer<T>::CopyFromSystemMemory(const std::vector<T> &bufferInSystemMemory)
			{
